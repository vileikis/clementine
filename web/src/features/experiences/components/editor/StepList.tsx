 ...
 * Component: StepList
 *
 * Displays a draggable list of experience steps with selection state.
...
export function StepList({
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const result = await reorderSteps(experienceId, newOrderIds);
  ...
  const handleTypeSelected = async (type: StepType) => {
    const result = await createStep(experienceId, type);
  ...
  const handleDuplicate = async (step: Step) => {
    const result = await duplicateStep(experienceId, step.id);
    if (result.success && result.stepId) {
      onSelectStep(result.stepId);
    }
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    const result = await deleteStep(experienceId, stepToDelete.id);
    if (result.success) {
      ...
      <AlertDialog open={!!stepToDelete} onOpenChange={() => setStepToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete &ldquo;{stepToDelete?.title || "Untitled"}&rdquo; from the experience.
