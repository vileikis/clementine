...
/**
 * Component: ShortTextEditor
 *
 * Editor for Short Text step type.
 * Configures single-line text input with variable name, placeholder, max length, and required flag.
 */
...
import type { StepShortText } from "../../types";

const shortTextFormSchema = z.object({
  // Base fields
...
type ShortTextFormValues = z.infer<typeof shortTextFormSchema>;

/** Fields to compare for changes */
const FIELDS_TO_COMPARE: (keyof ShortTextFormValues)[] = [
  "title",
  "description",
  "mediaUrl",
  "mediaType",
...
export function ShortTextEditor({
  ...
  const config = step.config ?? {
    ...
    maxLength: STEP_CONSTANTS.DEFAULT_SHORT_TEXT_MAX_LENGTH,
    required: false,
  };
  ...
  useEffect(() => {
    ...
    const subscription = form.watch((values) => {
      onPreviewChange(values as ShortTextFormValues);
    });
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Session variable to store the user&apos;s answer
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
            ...
            <FormItem>
              ...
              <FormControl>
                <Input
                  type="number"
                  min={1}
                  max={STEP_CONSTANTS.MAX_SHORT_TEXT_LENGTH}
                  {...field}
                  onChange={(e) => field.onChange(parseInt(e.target.value, 10) || 1)}
                />
              </FormControl>
              <FormDescription>
                Maximum number of characters allowed (1-{STEP_CONSTANTS.MAX_SHORT_TEXT_LENGTH})
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
          ...
          render={({ field }) => (
            <FormItem className="flex items-center justify-between rounded-lg border p-3">
              <div className="space-y-0.5">
                ...
                <FormDescription>
                  User must provide an answer to continue
                </FormDescription>
              </div>
              <FormControl>
                <Switch
