...
- [ ] T025 [US2] Update `apps/clementine-app/src/domains/experience/create/hooks/useRefMediaUpload.ts` — change `outcome.imageGeneration.refMedia` reads to `outcome.aiImage.imageGeneration.refMedia`. Update `outcomeRef` usage to read from the new config path. Ensure `onMediaUploaded` callback adds to the correct field.
