 ...
 * 2. Update project overlays config (via useUpdateOverlays)
 *
...
/**
 * Composition hook: Upload overlay asset + update project config
 *
 * Combines upload and config update into a single mutation that is
 * tracked by the project config designer save indicator. This ensures users
 * see a single save operation for the entire flow.
 *
 * Architecture:
 * - Uses useUploadMediaAsset (domain-agnostic, no project coupling)
 * - Uses useUpdateOverlays (project-specific config update)
